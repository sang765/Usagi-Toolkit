...
- Claude Desktop environment — GUI-based AI assistant.
- Uses platform-specific config directories for MCP configuration:
  - macOS: `~/Library/Application Support/Claude/claude_desktop_config.json`
  - Linux: `~/.config/Claude/claude_desktop_config.json`
- Skills are NOT supported in Claude Desktop — only MCP tools are available.
- Claude Desktop has a graphical interface, not terminal-based.
...
- Claude Desktop is a GUI application — terminal commands may not work directly.
- MCP servers must be configured in platform-specific config directory.
- Skills are NOT supported in Claude Desktop — only MCP tools are available.
- Claude Desktop may have different file system access than terminal-based harnesses.
...
3. Has it actually been built/tested/verified, or am I assuming it works?
4. Is the style consistent with the rest of the codebase?
5. Is there anything that needs user confirmation before proceeding?
