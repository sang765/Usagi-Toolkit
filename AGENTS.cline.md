...
- Cline environment — VS Code extension for AI coding assistance.
- Uses `~/.cline/data/settings/cline_mcp_settings.json` for MCP server configuration.
- Skills are NOT supported in Cline — only MCP tools are available.
- Cline operates within VS Code's file system and terminal.
...
- Cline is a VS Code extension — it operates within VS Code's file system.
- MCP servers configured in `~/.cline/data/settings/cline_mcp_settings.json`.
- Skills are NOT supported in Cline — only MCP tools are available.
- Cline may have different permissions than terminal-based harnesses.
