...
4. Read `references/access-classification.md` and classify each capability (list/search, details, chapter list, chapter pages, image delivery) as **public**, **authenticated**, **paywalled/subscription**, **blocked-by-terms**, or **unknown** using `tsuki_classify_access` before researching further. This decides what is buildable — do it before spending effort on endpoints you cannot legally use. Treat low-confidence automated results as requiring manual review.
